 ...
 * @author Modibedi Mosigi (101256959)
 *
 */

#include "interrupts_101318070_101256959.hpp"
...
std::tuple<std::string, std::string, int> simulate_trace(std::vector<std::string> trace_file, int time, std::vector<std::string> vectors, std::vector<int> delays, std::vector<external_file> external_files, PCB current, std::vector<PCB> wait_queue) {
    ...
    for(size_t i = 0; i < trace_file.size(); i++) {
        ...
        if(activity == "CPU") { //As per Assignment 1
            ...
            int fork_time = duration_intr;
            execution += std::to_string(current_time) + ", " + std::to_string(fork_time) + ", cloning the PCB\n";
            ...
            //Empty scheduler as per guidelines
            ...
            system_status += "time: " + std::to_string(current_time) + "; current trace: FORK, " + std::to_string(fork_time) + "\n";
            ...
            auto [child_execution, child_system, child_time] = simulate_trace(child_trace, current_time, vectors, delays, external_files, child, wait_queue);

            //Append to the variables
            execution += child_execution;
            system_status += child_system;
            current_time = child_time;

            free_memory(&child);

            //Parent out of waiting
            if(!wait_queue.empty()){
            ...
            //The times below were taken from the test templates
            ...
            auto [exec_execution, exec_system, exec_time] = simulate_trace(exec_traces, current_time, vectors, delays, external_files, current, wait_queue);

            system_status += exec_system;
